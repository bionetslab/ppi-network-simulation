...
def fun(parameters,f,pl_num,t,final_table,i,dir_output):
  ...
  node_degrees_simulated, num_ppis_simulated, node_degrees_ground_truth, num_ppis_ground_truth = ppinetsim.run_simulation(parameters, verbose=True)
  # remove zero
  node_degrees_simulated = node_degrees_simulated[node_degrees_simulated != 0]
  np.savetxt(dir_output + 'degree_dist_file/'+'degree_observed_'+str(i)+'_'+ str(f.replace('.json','')) +'.txt', node_degrees_simulated, delimiter='\n')
  len(node_degrees_simulated)
  r = callPoweRlaw(node_degrees_simulated,pl_num,t)
  r.append(f)
  final_vector = [parameters.num_proteins, parameters.num_ppis_ground_truth, parameters.false_positive_rate, parameters.false_negative_rate, parameters.generator, parameters.biased, parameters.baseline_degree, parameters.test_method, parameters.acceptance_threshold, parameters.num_studies, parameters.sample_studies, parameters.only_big_studies,parameters.pm_num_baits,parameters.pm_num_preys, num_ppis_simulated, len(node_degrees_simulated), i]
  for j in r:
    final_vector.append(j)
  final_table.append(final_vector)
...
def run_iterations_withParallel(files,n,dir_parameters,pl_num,t,dir_output,name_file,jobs):
  ...
  for f in files:
    ...
    for element in final_table:
      temp=pd.DataFrame(element, columns=['num_proteins','num_ppis_ground_truth','false_positive_rate','false_negative_rate','generator','biased','baseline_degree','test_method','acceptance_threshold','num_studies','sample_studies','only_big_studies','pm_num_baits','pm_num_preys','num_ppis_simulated','num_proteins_simulated','n_iter','pvalue','xmin','pars','ntail','plCat','type'])
